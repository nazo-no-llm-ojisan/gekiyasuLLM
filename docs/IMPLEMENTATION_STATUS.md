...
最終更新: 2026-07-13（Issue #12監査完了・closed。M2完了。main CI green）
...
| M2 model identity | **済** — #12でraw/normalized provider、access、canonical key、rule保守契約を監査完了 |
...
| M2全体 | **済** — #12/#13/#16/#14/#15完了、main CI green |
...
| 本番利用 | **不可** — M3署名/DNS pin未完、Phase 3+ publication governance未完 |
| CI | **green** — main GitHub Actions `ci` 成功。recursive test discovery、proxy build + dist smokeを実行。Node.js 20 deprecation annotationはmaintenance debt |
...
```text
#12 model-id contract review                         完了
...
feature実装 `a4c1b81` とmain統合 `6e11a83` を監査し、次を確認した。

- `rawProvider`は入力provider断片を保持し、providerなしは`unknown`
- `normalizedProvider`はmarker除去・trim・小文字化・既存alias適用後の値
- deprecated `provider`は`rawProvider`と常に同値で互換維持
- direct providerはnormalized provider、hosted/unknownはfamily mappingからdeveloper解決
- `:chat` / `:instruct`とterminal `-chat` / `-instruct`を`accessVariant`へ保持
- colon-less access抽出をregion/date/derivativeより先に行う
- canonical keyは正確に`developer|family|version|derivative`で、access/provider/regionを含めない
- suffix抽出でfamilyが空・不正になる場合はfail-safeに元familyを保持
- rule表はpure/offline TypeScript定数のままとし、追加基準・必須fixture・推測禁止をコードとdesign 06へ記載
- public型、実装、unit fixture、生成型宣言、design 06が一致
- proxy hot path、feed generator、catalogへ変更を広げていない

統合後のroot testはschema 39/39、proxy 150/150、Windows script 1/1、catalog 6/6。schema typecheck/buildと`git diff --check`も成功。GitHub Actions run `29253627169`はmain SHA `6e11a83`でsuccess。blocking findingなし。
...
| 順 | 作業 | 状態 |
|---|---|---|
| 完了 | [#12 model-id normalization contract](https://github.com/nazo-no-llm-ojisan/gekiyasuLLM/issues/12) | **done / closed** |
...
M2の全契約・生成・consumer・actual path・static catalog監査は完了した。次の本線はPhase 3+ governanceとM3 security gateであり、M2完了を本番利用可能と混同しない。
...
| T-041 | optional Lua hook | T-039確定済み・別途優先度判断 |
...
- main SHA `6e11a83`のGitHub Actions run `29253627169`はsuccess
- M2全体は完了
- 本線は Phase 3+ / M3 → Phase 4
