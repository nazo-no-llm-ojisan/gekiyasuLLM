...
- 開発ループ: `docs/BRAINLESS_TDD.md`（**赤 → 緑 → コミット**）
- 並列: `docs/PARALLEL_AGENTS.md` — **設計は直列、契約済み実装は並列**。台帳の `owned_paths` 外を触るな。`contract_changes: proposed` は直列のみ。
- 1エージェント = 台帳の **赤テスト1本**。契約（`packages/schema` 公開型等）を無断変更しない。
- ユーザが「通った／通らない／次の一手」だけ返してきたら **次の一手だけ**。
- 講義が必要ならファイルに書いてパスだけ。
