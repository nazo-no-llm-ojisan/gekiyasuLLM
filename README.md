...
**Status:** design docs + early proxy. Default listen **`127.0.0.1:16191`**. MVP passthrough lives in `packages/proxy`. Not production-ready yet.
...
**現状:** 設計 + 初期 Proxy。既定待ち受け **`127.0.0.1:16191`**。透過 Proxy は `packages/proxy`。まだ本番利用向けではありません。
