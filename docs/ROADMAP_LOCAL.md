...
L5  Executor が plan.primary     ░░░░ 次（T-023）
L6  複数候補 + hard filter       ░░░░ 未
...
**ピン:** **L4 完了 → 次は L5（Executor = plan.primary）**  
中継そのものは L1 でコード上できる。L11 は実キーの手動確認。
...
| L5 Executor primary | **T-023** | todo ← **いまの次** |
...
```text
1. T-023  Executor が plan.primary を使う     ← 次の一手
2. L11    実キーで中継 E2E（任意・手動）
3. L6     候補2本 + hard filter
