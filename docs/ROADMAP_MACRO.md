...
[==== YOU ARE HERE ====]
  ローカル本線: M1 完了 → M2 → M3（[ROADMAP_LOCAL](./ROADMAP_LOCAL.md)）
...
**一言:** 中継は使える（L12）。M1（正しく振り分ける）完了。次は **M2 データ縦貫通**（T-039/024/050/051）。公開自動フィードは **M3**（署名 + DNS pin）。  
...
| **3+** | ローカル本線 M1–M3 | 正しく振り分け → データ縦貫通 → 安全な自動公開 | **M1 完了**。次は M2（詳細は ROADMAP_LOCAL） |
