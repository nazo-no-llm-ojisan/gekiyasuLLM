...
| NFR-12 | 観測対象への反作用 | 公開ヘッドラインやランキングが traffic を動かし、可用性観測を自己参照的に変えるリスクを明示する。ローカル実行・ユーザ別 policy・説明可能な ranking を herding 抑制装置として維持する |
...
┌─────────────────────────────────────────────────────────────┐
│  IDE / Agent / SDK                                          │
│   baseURL = http://127.0.0.1:16191/v1                       │
└───────────────────────────┬─────────────────────────────────┘
                            │ OpenAI-compatible
┌───────────────────────────▼─────────────────────────────────┐
...
### 10.2.1 観測対象への反作用（herding）

公開ヘッドラインや「安い / 動く」ランキングは、利用者 traffic を特定 offering に寄せ、観測している可用性そのものを変える可能性がある。これは価格・可用性サービスが持つ自己参照リスクであり、精度だけでなく表示とルーティング設計の問題として扱う。

MVP では、中央が単一の最適解を配らず、各ユーザーのローカル Proxy が cost / trust / allowlist / private mode に従って選ぶことで集中を緩和する。将来、重み付きスコアや bandit を入れる場合も、説明可能性を落として traffic を一極集中させないことを確認する。
