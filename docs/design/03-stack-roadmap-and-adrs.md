...
- **採用**: フィード必須の機械可読 COI（`sponsored`, `affiliate`, `editorial_rank_influence: "none"`）+ UI 分離 + 既定ソート非干渉
- **却下**: サイト表示のみの開示、価格データへの暗黙バイアス
...
- **条件**: 秘密を入れない、保証しない免責を README に置く、第三者への曖昧な貶め表現を避ける、ライセンス・COI・訂正方針を公開する
- **却下**: private で構想だけ温める（公開時の書き換えコストと vaporware 感が残る）
- **補足**: 不透明な仲介を扱う側が不透明では成立しない。「信用して」ではなく **検査できる** 状態を維持する

### ADR-013: ライセンス

- **採用**: **Apache-2.0**（Proxy・ツール・スキーマ・フィード成果物）
- **商標**: 名称・ロゴは `TRADEMARKS.md` で分離（Apache は商標を原則付与しない）
- **却下**: 全体 MIT のみ（特許条項の明確さを優先して Apache）、編集データだけ完全クローズ（検査可能性と矛盾）

### ADR-014: 利益相反フィールド

- **採用**: provider ごとに `sponsored`, `affiliate`, `editorial_rank_influence`（既定 `"none"`）を **フィード必須**
- **却下**: UI だけのバッジ、ランキングへの暗黙ブースト
...
1. 正式名称・ドメイン取得可否（gekiyasuLLM.com）と商標の法的登録要否
...
5. ~~ライセンス~~ → **確定: Apache-2.0 + TRADEMARKS.md**（ADR-013）
6. 日本の景表法・アフィリエイト表示の **具体的な画面文言**（弁護士レビュー前。フィールド形状は ADR-014 で固定）
...
- ローカル JSON 読込
- サンプル 3–5 モデル（各 provider に COI 必須フィールド）
- `feed_version` と空でもよい `corrections[]`
- `feed pull`（未署名）
