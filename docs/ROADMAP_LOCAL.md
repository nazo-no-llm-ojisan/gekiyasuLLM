...
L7  fallback / circuit           ░░░░ 次（plan.fallbacks を Executor で試す）
...
**ピン:** **L6 完了 → 次 L7（fallback 実行）または L9 / L11**
...
```text
1. ~~T-023 / L6 filter+rank~~ done
2. L7     fallback を Executor で実行   ← 次の一手（本線）
3. L9     CostEstimate（input/output だけ）
4. L11    実キー E2E（任意）
5. L8     静的フィード pull
6. L10    ローカル統計
```
